              ...
              {todayEntries.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {todayEntries.map((entry, i) => {
                    ...
                    return (
                      <Card key={i} className={`flex gap-4 items-start border ${color.border} ${color.darkBorder} hover:shadow-md transition-all relative overflow-hidden`}>
                        <div className={`w-11 h-11 rounded-xl ${color.bg} ${color.darkBg} flex items-center justify-center shrink-0`}>
                          <span className={`material-symbols-outlined ms-fill ${color.text} ${color.darkText} text-[24px]`}>school</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-on-surface truncate">{entry.subject}</p>
                          <p className={`text-xs ${color.text} ${color.darkText} font-semibold mt-0.5`}>{entry.code}</p>
          ...
          {view === "overview" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.keys(groupedOverview).map((subjName, i) => {
                ...
                return (
                  <Card key={i} className={`border ${color.border} ${color.darkBorder} flex flex-col gap-3 hover:shadow-md transition-all`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl ${color.bg} ${color.darkBg} flex items-center justify-center shrink-0`}>
                        <span className={`material-symbols-outlined ms-fill ${color.text} ${color.darkText} text-[20px]`}>menu_book</span>
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-on-surface truncate">{subjName}</p>
                        <p className={`text-xs ${color.text} ${color.darkText} font-semibold`}>{code}</p>
                      </div>
                    </div>
                      ...
                      {gridDays.map((day) => {
                        ...
                        return (
                          <td key={day} className="px-2 py-2">
                            {entry && color ? (
                              <div className={`${color.bg} ${color.darkBg} border ${color.border} ${color.darkBorder} rounded-lg px-2 py-1.5 text-left`}>
                                <p className={`text-xs font-bold ${color.text} ${color.darkText} leading-tight truncate`}>{entry.subject}</p>
