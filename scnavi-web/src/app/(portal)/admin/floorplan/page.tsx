...
export default function FloorPlanBuilderPage() {
  ...
  return (
    <>
      {/* Mobile Guard */}
      <div className="flex md:hidden flex-col items-center justify-center h-[calc(100vh-10rem)] text-center px-6 gap-4">
        <span className="material-symbols-outlined text-[64px] text-outline-variant">desktop_windows</span>
        <h2 className="font-headline font-semibold text-xl text-on-surface">Desktop Required</h2>
        <p className="text-sm text-outline max-w-xs">
          The Floor Plan Builder requires a desktop browser for precision drawing tools. Please switch to a larger screen.
        </p>
      </div>

      {/* Desktop Builder */}
      <div className="hidden md:flex flex-col h-[calc(100vh-7rem)] animate-fade">
      {/* ── Header ─────────────────────────────────────── */}
