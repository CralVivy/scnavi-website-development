...
export default function MapPage() {
  ...
  const handleSelectBuilding = async (b: Building) => {
    ...
    if (b.rooms) return; // already loaded

    setLoadingRooms(true);
    try {
      const bDoc = await getDoc(doc(db, "buildings", b.id));
      if (bDoc.exists()) {
        const data = bDoc.data();
        const updated = { ...b, rooms: data.rooms || [] };
        
        // Update local list
        setBuildings(prev => prev.map(pb => pb.id === b.id ? updated : pb));
        setSelectedBuilding(updated);
      } else {
        const updated = { ...b, rooms: [] };
        setBuildings(prev => prev.map(pb => pb.id === b.id ? updated : pb));
        setSelectedBuilding(updated);
      }

      // Also try to load floor plan for ground floor
      try {
        const fpDoc = await getDoc(doc(db, "buildings", b.id, "floors", "ground"));
        if (fpDoc.exists() && fpDoc.data().elements) {
          setFloorPlanElements(fpDoc.data().elements);
        }
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col space-y-0 -m-4 md:-m-6 lg:-m-8 animate-fade">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Room Detail Panel */}
        {selectedBuilding && (
          <aside className="w-80 xl:w-96 bg-surface-container-lowest border-l border-outline-variant/20 flex flex-col overflow-hidden">
